...
const StatsDisplay = ({ stats }: { stats: GameStats }) => (
  <div className="bg-gray-800 rounded-xl p-4 shadow-lg border border-gray-700 w-full">
...
// Component: Legend
const Legend = () => (
  <div className="bg-gray-800 rounded-xl p-4 shadow-lg border border-gray-700 text-xs text-gray-400 w-full">
...
const App = () => {
  ...
  if (isMobile) {
    return (
      <div className="h-screen w-full flex flex-col bg-gray-900 overflow-hidden">
        ...
        {/* Header */}
        <header className="h-14 flex items-center justify-between px-4 bg-gray-900 border-b border-gray-800 shrink-0 z-20 shadow-md">
        ...
        <div className="flex-grow relative w-full overflow-hidden bg-gray-900">
           {/* Message Toast */}
           {message && (
             <div className="absolute top-4 left-0 right-0 mx-auto w-max max-w-[90%] bg-black/80 backdrop-blur text-white px-5 py-3 rounded-xl border border-gray-600 shadow-2xl text-center font-bold z-30 pointer-events-none animate-in fade-in slide-in-from-top-4">
               {message}
             </div>
           )}
           ...
           {mode === GameMode.MENU && (
             <div className="absolute inset-0 bg-gray-900/95 z-40 flex flex-col items-center justify-center p-6 gap-4">
                 <h2 className="text-3xl font-bold text-white mb-4">New Game</h2>
                 <button onClick={start1P} className="w-full bg-blue-600 py-4 rounded-xl text-white font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-transform">
                     <Bot /> 1 Player
                 </button>
                 <button onClick={start2P} className="w-full bg-purple-600 py-4 rounded-xl text-white font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-transform">
                     <Users /> 2 Players
                 </button>
        ...
        {/* Controls Footer */}
        {mode !== GameMode.MENU && (
          <div className="bg-gray-800 border-t border-gray-700 p-4 shrink-0 z-20 flex flex-col gap-3 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.3)]">
             
             {/* Hint Area - Dedicated Row */}
             {hint && (
               <div className="w-full bg-indigo-900/40 border border-indigo-500/30 p-3 rounded-xl text-sm text-indigo-200 animate-in fade-in slide-in-from-bottom-2">
             ...
             <div className="flex gap-3 mt-1">
                {mode === GameMode.PLAYING && (
                    <>
                        <button 
                            ...
                            className={`flex-grow h-14 rounded-xl flex items-center justify-center gap-2 shadow-lg transition-all font-bold text-lg ${isRowFull ? 'bg-green-600 active:bg-green-700 text-white' : 'bg-gray-700 text-gray-500'}`}
                        >
                        ...
                        <button 
                            ...
                            className={`w-16 h-14 rounded-xl flex items-center justify-center shadow-lg transition-all ${(!loadingHint && (!hint || hint === GEMINI_ERROR) && canAskHint && !isAnimating) ? "bg-indigo-600 hover:bg-indigo-500 text-white" : "bg-gray-700 text-gray-500 cursor-not-allowed opacity-50"}`}
                        >
                ...
                {mode === GameMode.SETUP_2P && (
                     <button 
                        ...
                        className="w-full bg-red-600 active:bg-red-700 text-white h-14 rounded-xl font-bold text-lg shadow-lg"
                    >
                        LOCK CODE
                    </button>
                )}
                ...
                {mode === GameMode.GAME_OVER && (
                     <button 
                        ...
                        className="w-full bg-blue-600 active:bg-blue-700 text-white h-14 rounded-xl font-bold text-lg flex items-center justify-center gap-2 shadow-lg"
                    >
        ...
        {isMenuOpen && (
            <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex justify-end animate-in fade-in duration-200">
                <div className="w-4/5 h-full bg-gray-800 p-6 flex flex-col gap-6 shadow-2xl animate-in slide-in-from-right duration-300">
                    ...
                    <button 
                        onClick={(e) => handleNewGame(e)} 
                        className={`flex items-center gap-3 w-full p-4 rounded-lg text-white font-semibold transition-colors mt-4 ${confirmReset ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-700 hover:bg-gray-600'}`}
                    >
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-start md:justify-center p-4 pb-20 overflow-x-hidden">
      <h1 className="text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-600 mb-6 tracking-tight flex-shrink-0">
        MIND MASTER
      </h1>
      ...
      <div className="flex flex-col md:flex-row gap-8 items-start justify-center w-full max-w-6xl">
        <div className="relative w-full md:w-[450px] md:h-[750px] flex-shrink-0 flex justify-center bg-gray-900 rounded-2xl shadow-2xl border-4 border-gray-800 overflow-hidden">
           ...
           {mode === GameMode.MENU && (
            ...
            {message && (
              <div className="absolute top-32 left-0 right-0 mx-auto w-max max-w-[90%] bg-black/80 backdrop-blur text-white px-4 py-2 rounded-full border border-gray-700 shadow-xl text-center font-semibold animate-fade-in-down z-20 pointer-events-none">
                {message}
              </div>
            )}
        </div>
        ...
        <div className="w-full md:w-64 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3">
            ...
            {mode === GameMode.SETUP_2P && (
              <button onClick={submit2PSecret} className="w-full bg-red-600 hover:bg-red-500 text-white font-bold py-3 rounded-xl shadow-lg transition-colors flex items-center justify-center gap-2">
            ...
            {(mode === GameMode.GAME_OVER || mode === GameMode.PLAYING) && (
               <button 
                  onClick={(e) => handleNewGame(e)} 
                  className={`w-full font-bold py-3 rounded-xl shadow-lg transition-colors flex items-center justify-center gap-2 mt-4 text-white ${confirmReset ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-700 hover:bg-gray-600'}`}
               >
